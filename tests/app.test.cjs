...
test('index and all text shards are consistent, source URLs are HTTPS, release versions agree',()=>{
 ...
 const gl12=data.docs.find(d=>d.id==='GL12');
 const gl73=data.docs.find(d=>d.id==='GL73');
 assert.equal(gl12.yr,'2025');assert.match(gl12.t,/2025/);
 assert.equal(gl73.yr,'2025');assert.match(gl73.t,/2025/);
});
